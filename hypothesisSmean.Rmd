...
_ To formulate the decision rule, a good idea is to draw a normal density and to specify the rejection region, as follows:

![Normal and Rejection areas](Normal_Rejection.png)
- From the above graph, we can see that we have two rejection regions. The cut-off (border) of the rejection region start at 
$Z_{\alpha= 1.96}$ at the right side, and at $- Z_{\alpha= - 1.96}$ at the left side. That is, the rejection regions are the red areas. 

### Greate! But what does this mean?

#### Good questions! Here is the answer:
...
- $$ If \ Z_{test} > Z_{\alpha} = 1.96 \rightarrow \  Reject \ H_0$$

- or $$ If \ - Z_{test} < - Z_{\alpha} = - 1.96 \rightarrow \  Reject \ H_0$$
...
- This part of the decision rule said that: If the value of the $Z_{test}$ larger than the right hand-side border of the rejection area, 
...
- This part of the decision rule said that: If the value of the $- Z_{test}$ smaller than the left hand-side border of the rejection area, 
...
_ Now we need to find and compare the $Z_{test}$ with the $Z_{\alpha}$. 

- The following graph shows the value of the $Z_{test} = -2.36$. From the graph, the $Z_{test} = -2.36$ falls inside the left hand-side
rejection area. 

### Ok. What does this means?

- As explained above, it means that the $Z_{test} = -2.36$ is smaller than the $Z_{\alpha}= -1.96$. In other words, the $Z_{test}$ is inside the
rejection region. Hence, based on the decision rule, we need to reject $H_0$. 

![Normal](final_Sou.Nor.png)

### Conclusion 

- Because the $Z_{test} = -2.36$ is smaller than the $Z_{\alpha}= -1.96$, and hence falls in the rejection region, we reject $H_0$, and conclude that, the population mean is not equal to $200$. 
